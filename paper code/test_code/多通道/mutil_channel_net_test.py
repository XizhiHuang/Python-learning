...
root_dir = "D:/codetest/mutilchannel/"
...
label_colors_img = os.path.join(root_dir, "label/").replace('\\', '/')
label_colors_jpg = os.path.join(root_dir, "jpg/").replace('\\', '/')
...
if __name__ == '__main__':
    ...
    Model_Path = 'D:/codetest/mutilchannel/unet_model_ep10_dp0.4.pth'
    ...
    with torch.no_grad():
        for data in val_data_loader:
            ...
            oimg_root=tmp_root.replace('raw', 'result')
            oimg.save(oimg_root)
            ...
            split_path[0]=split_path[0].replace('img','label')
            split_path = split_path[0] + '.png'
